...
@app.route('/department/delete/<int:cid>')
@login_required
def delete_department(cid):
    d = Department.query.filter_by(cid=cid).first()
    if d:
        db.session.delete(d)
        db.session.commit()
...
def addattendance():
    ...
    if request.method=="POST":
        ...
        db.session.add(atte)
        # add trigger entry
        trig=Trig(rollno=rollno, action=f'Attendance added ({attend})', timestamp=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        db.session.add(trig)
        db.session.commit()
...
def delete(id):
    ...
    # record trigger before deletion
    try:
        trig=Trig(rollno=post.rollno, action='Student deleted', timestamp=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        db.session.add(trig)
    except Exception:
        pass
...
def edit(id):
    ...
    if request.method=="POST":
        ...
        # add trigger for edit
        trig=Trig(rollno=rollno, action='Student edited', timestamp=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        db.session.add(trig)
        db.session.commit()
...
def addstudent():
    ...
    if request.method=="POST":
        ...
        dup = Student.query.filter_by(rollno=rollno).first()
        if dup:
        ...
        db.session.flush()
        # add trigger for new student
        trig=Trig(rollno=rollno, action='Student added', timestamp=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        db.session.add(trig)
        db.session.commit()
